...
export class ProfileService {
  ...
  public getUser(){ //get all users
    return this.httpClient.get("http://localhost:19090/booky/users/", this.httpOptions);
  }

  public getUserById(id){ //get user by id is usefull to git user's profile
    return this.httpClient.get("http://localhost:19090/booky/users/" + id, this.httpOptions);
  }

  public getUserTypeByUserId(id){ //get user type by user id
    return this.httpClient.get("http://localhost:19090/booky/users/type/" + id, this.httpOptions);
  }

  public updateUserInformation(id, user) { //update user information by id 
    const url = 'http://localhost:19090/booky/users/' + id;
